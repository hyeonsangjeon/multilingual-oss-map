...
## D21 — Ship `langgap`: turn the finding into a tool anyone can run on their own repo

The project had *proven* the README↔issue language gap at 40M‑repo scale but offered nothing a visitor
could **run**. To convert interest into reuse, added **`tools/langgap`** — a dependency‑light CLI + a
composite **GitHub Action** + a **badge** that detects any repo's README vs issue language and places
the result in this project's landscape.

- **Reuse, not re‑derive.** The tool reads the committed `site/data/*.json` (regenerated by
  `scripts/aggregate.py`) for ranks, the per‑language "no non‑English README" gap share, and the paired
  agreement — so its numbers stay byte‑identical to the site; no second source of truth.
- **Same method as the map (D3/D6/D9).** Detectors are kept *separate* and a label is accepted only when
  **≥ strictness** agree. Two pure‑Python detectors (lingua + langid) install anywhere (strictness ≤ 2);
  **fasttext is optional** for a true all‑3 runtime consensus. Runtime agreement caps at the number of
  active detectors, while the *landscape* comparison defaults to **all‑3** (matching D7/D9 and GitHub's
  published tables) — so even the 2‑detector install reports the headline all‑3 figures.
- **Honesty guard (D4/D15).** An English/absent README is reported as *"no non‑English README
  classification,"* never "written in English"; snippets use the dataset's first‑150‑chars, most‑commented
  basis.
- **Robust + offline‑testable.** `--readme` / `--issues-text` need no network; `--repo` uses the GitHub
  API (`GITHUB_TOKEN` if present). `fasttext-wheel`'s NumPy‑2 incompatibility is worked around by calling
  its C binding directly, and every detector is self‑tested at startup so a broken one is auto‑dropped.
- **Badges = the viral loop.** A self‑contained **SVG** (no external dependency) plus a **shields.io**
  endpoint JSON; a badge on someone else's repo backlinks here (orange = gap, green = aligned).
- **Verified end‑to‑end:** EN‑README/KO‑issues reproduces the site's headline at all‑3 (Korean **#1** in
